...
lazy_static! {
    static ref RECORDER: Arc<AsyncMutex<AudioRecorder>> =
        Arc::new(AsyncMutex::new(AudioRecorder::new()));
    static ref RECORDING_POPUP: Arc<AsyncMutex<RecordingPopupHandler>> =
        Arc::new(AsyncMutex::new(RecordingPopupHandler::new()));
}
...
pub async fn is_recording() -> bool {
    ...
    recorder.recording.load(Ordering::Acquire)
}

async fn check_recorder_async(handle: AppHandle<Wry>) -> Result<()> {
    let lock = RECORDER.clone();
...
pub async fn start_shortcut() -> Result<()> {
    let config = {
        let manager = voice_recognition::VOICE_REC_CONFIG_MANAGER.read().await;
        manager.get_config()
    };
    if config.enable && !config.record_key.is_empty() {
...
pub fn update_shortcut(
    original: &VoiceRecognitionConfig,
    new_config: &VoiceRecognitionConfig,
) -> Result<()> {
    let app = app::get_app_handle()?;
    let mut manager = app.global_shortcut_manager();

    // if original registered, unregister it
    if original.enable && !original.record_key.is_empty() {
    ...
    if new_config.enable && !new_config.record_key.is_empty() {
...
impl RecordingPopupHandler {
    ...
    fn display(&self) -> Result<()> {
        if self.window.is_none() {
            return Err(anyhow!("Init popup window first"));
        }
    ...
    fn close(&self) -> Result<()> {
        ...
        self.window
            .as_ref()
            .ok_or(anyhow!("Failed to get window reference"))?
            .hide()?;
        Ok(())
    }
}

async fn open_recording_popup(app: &AppHandle<Wry>) -> Result<()> {
...
async fn close_recording_popup() -> Result<()> {
    let lock = RECORDING_POPUP.clone();
