...
fn build_proxy(config: HttpProxyConfig) -> Result<Option<Proxy>> {
    if !config.enable {
        return Ok(None);
    }
...
pub async fn new_http_client() -> Result<reqwest::Client> {
    let manager = proxy::HTTP_PROXY_CONFIG_MANAGER.read().await;
...
pub async fn get_json<T>(url: String) -> Result<T>
where
    T: serde::de::DeserializeOwned,
{
    ...
    if res.status() == StatusCode::OK {
        let res_json: String = res.text().await?;
        Ok(serde_json::from_str::<T>(&res_json)?)
    } else {
        Err(anyhow!(
            "do get json request with status: {}, error: {}",
            res.status(),
            res.text().await?
        ))
    }
...
pub async fn download(url: String, download_file: PathBuf) -> Result<()> {
    let client = new_http_client().await?;
